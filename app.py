...
st.title("⚕️ Прогнозирование летального исхода")
st.markdown("Введите показатели пациента при поступлении для расчета вероятности неблагоприятного исхода.")
...
if st.button("Рассчитать риск", type="primary", use_container_width=True):
    ...
    if probability < prob_low:
        ...
        risk_color = "green"
        risk_emoji = "✅"
        recommendation = "Возможно продолжение наблюдения в инфекционном отделении, отказ от избыточных инвазивных процедур."
    elif probability >= prob_high:
        risk_group = "ВЫСОКИЙ РИСК"
        risk_color = "red"
        risk_emoji = "🔴"
        recommendation = "Рекомендуется немедленная госпитализация в ОРИТ, проведение инвазивного мониторинга, интенсивной терапии."
    else:
        risk_group = "УМЕРЕННЫЙ РИСК"
        risk_color = "orange"
        risk_emoji = "⚠️"
        recommendation = "Необходима госпитализация с интенсивным мониторингом, готовность к эскалации терапии."
        
    st.markdown(f"<h2 style='text-align: center; color: {risk_color};'>{risk_emoji} {risk_group}</h2>", unsafe_allow_html=True)
    
    st.info(f"**Рекомендация по маршрутизации:**\n\n{recommendation}")

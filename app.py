...
if st.button("Рассчитать риск", type="primary", use_container_width=True):
    ...
    if probability < prob_low:
        ...
        risk_color = "red"
        risk_emoji = "🔴"
        recommendation = "Продолжить наблюдение в ОРИТ. Рекомендуется смена антибактериальной терапии. Рекомендуется экстренное проведение неровизуализации для исключения осложнений, дополнительного оследования для исключения других очагов инфекционного процесса. Расширение объема интенсивной терапии, применение экстракорпоральных методов лечения. Интенсивный лабораторный мониторинг, оценка САД"
    else:
